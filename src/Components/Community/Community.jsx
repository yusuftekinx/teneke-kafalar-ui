...
function Community({ user, ...props }) {
    ...
    useEffect(() => {
        if (searchText !== '') {
            ...
            searchRequest(props.token, searchText).then(response => {
                ...
                toast.error('Bir hata oluştu.Sayfayı yenileyin.')
            })
        }
        else {
            setSearchBar(false)
    ...
    let onJoinCommunity = (e, id) => {
        ...
        setSearchBar(false)
        toast.success("Bir topluluğa katıldınız.")
    }
    ...
    return (
        <div className="community-component">
            ...
            <div className="community-top">
                ...
                {
                    user.role === 'admin' ? <button onClick={openModal} className="tkButton createCommunityBtn"><FaPlus />&nbsp;&nbsp;Topluluk Oluştur</button> : <form style={{ marginRight: "20px", display: 'flex', justifyContent: 'center', alignItems: 'center',width:'230px',paddingRight:"10px"}}><InputWithIcon placeholder="Bir topluluk ara... Ör: Teneke Kafalar" style = {{width:"230px"}} value={searchText} onChangeEvent={onSetSearchText} icon={<FaUserFriends />} required={true} /></form>
                }
            </div>
            <hr></hr>
                        ...
                        <div className="result-div">
                            {
                                searchResult.map(res => (
                                    <div className="result-community" key={res._id}>
                                        ...
                                        {
                                            res.members.indexOf(user.username) === -1 ? <button className="tkButton joinCommunityButton" onClick={(e) => { onJoinCommunity(e, res._id) }}>Katıl</button> : <span className="poppinsText" style={{ fontSize: "10px" }}>Katıldın</span>
                                        }
                                    </div>
                                ))
                            }
                ...
                <div className="myMembersCommunity">
                    {
                                ...
                                <span className="three-dot-icon" onClick={(e) => { setCommunityOptions(community._id) }}><BsThreeDots className="three-dot" /></span>
                                {
                                    user.role === 'admin' ? <span className="communityOptions" style={{ display: communityOptions === community._id ? 'flex' : 'none' }}>
                                        ...
                                        <span id="options-delete" onClick = {(e) => {sendLeaveRequestCommunity(e,community._id)}}>
                                            ...
                                            <span className="openSansText">Ayrıl</span>
                                        </span>
                                    </span>
                                }
                        ...
                        )) : <div className = "notFoundCommunity poppinsText"><AiFillWarning />&nbsp;&nbsp;Herhangi bir topluluğa katılmadın.</div>
                    }
            ...
            >
                ...
                <form onSubmit={sendCreateCommunityRequest}>
                    <InputWithIcon onChangeEvent={changeCommunityName} type={'text'} style={{ width: "88%" }} icon={<FaUserFriends />} placeholder={'Topluluk Adı'} isPasswordInput={false} htmlFor={'communityName'} />
                    <textarea value={communityContent} onChange={changeCommunityContent} className="mt20 openSansText" maxLength="500" placeholder="Topluluk Açıklaması" rows="5" cols="30" style={{ borderRadius: '8px', border: '1px solid lightgray', paddingLeft: "5px", paddingTop: "5px", outline: 'none' }}></textarea>
                    <br />
